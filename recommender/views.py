...
from .engine import RecommendationEngine, get_analytics_for_internship
from .serializers import RecommendationSerializer

# Create a global placeholder for our engine. It starts as None.
...
class InternshipAnalyticsView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        # LAZY LOADING: Initialize the engine if this is the first request.
        if recommendation_engine is None:
            print("Analytics view is initializing the engine...")
        ...
        # Fetch the internship details from the CORRECT, lazy-loaded engine instance
        internship = recommendation_engine.all_internships_map.get(internship_id)
        if not internship:
...
class RecommendInternships(APIView):
    def get(self, request, *args, **kwargs):
        ...
        # LAZY LOADING: Initialize the engine if this is the first request.
        if recommendation_engine is None:
            print("Recommendation view is initializing the engine...")
            recommendation_engine = RecommendationEngine()
        ...
        in_location_recs, global_recs = recommendation_engine.find_recommendations(skills, location, interest)
